...
Dashboard bawaan (Build command `npm run build`, Deploy command
`npx wrangler deploy`) sudah cukup — **gak perlu ubah setting apa-apa**.
Script `build` di `package.json` dijaga pakai env var `OPENNEXT_BUILDING`
supaya gak manggil dirinya sendiri berulang-ulang: panggilan pertama
menjalankan `opennextjs-cloudflare build` (yang butuh `next build` di
dalamnya), dan panggilan kedua — yang dipicu oleh tool itu sendiri — cuma
menjalankan `next build` polos karena env var-nya udah ke-set.

Set API key sekali sebagai secret:

```bash
npx wrangler secret put JKT48_API_KEY
```
